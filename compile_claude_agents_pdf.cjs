...
(async () => {
    console.log("Launching Puppeteer to build Claude Agents Carousel PDF...");
    ...
    const outDir = path.resolve(__dirname, './user_uploads/claude_agents_output');
    fs.mkdirSync(outDir, { recursive: true });

    for (let i = 1; i <= 7; i++) {
        const slidePath = `file://${path.resolve(__dirname, `./carousel-routine/temp/claude-agents-branded/slide-0${i}.html`)}`;
    ...
    await browser.close();
    console.log("Claude Agents Carousel PDF compilation complete!");
})();
