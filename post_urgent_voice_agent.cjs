...
(async () => {
  ...
  try {
    ...
    // Upload Carousel PDF
    const pdfPath = path.resolve(__dirname, 'voice_agent_carousel.pdf');
    ...
    if (titleInputHandle && titleInputHandle.asElement()) {
      ...
      await page.keyboard.type("AI Voice Calling Agent — Complete Blueprint", { delay: 10 });
    ...
    // Fill caption text
    const captionText = fs.readFileSync(path.join(__dirname, 'urgent_voice_agent_post.txt'), 'utf8');
    ...
    await delay(8000);
    console.log("🎉 AI VOICE CALLING AGENT CAROUSEL POST PUBLISHED SUCCESSFULLY TO LINKEDIN!");
