...
(async () => {
  ...
  try {
    ...
    if (titleInputHandle && titleInputHandle.asElement()) {
      ...
      await page.keyboard.type("How to Build a WhatsApp AI Agent", { delay: 10 });
    ...
    console.log("Waiting for main post composer editor to re-appear...");
    await waitForSelectorShadow(page, '.ql-editor, [contenteditable="true"]', 15000);
    await delay(1000);

    // Fill caption text
    const captionText = fs.readFileSync(path.join(__dirname, 'urgent_whatsapp_agent_post.txt'), 'utf8');
    ...
    await delay(8000);
    console.log("🎉 WHATSAPP AGENT CAROUSEL POST PUBLISHED SUCCESSFULLY TO LINKEDIN!");
