...
async function fillEditorText(page, text) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    editorHandle = await findShadowElement(page, (root) => {
      return root.querySelector('.ql-editor') || root.querySelector('div[contenteditable="true"]');
    });
    if (editorHandle && editorHandle.asElement()) break;
    await delay(500);
  }
  
  const editor = editorHandle ? editorHandle.asElement() : null;
...
(async () => {
  ...
  try {
    ...
    // Upload Carousel PDF
    const pdfPath = path.resolve(__dirname, 'claude_agents_carousel.pdf');
    ...
    if (titleInputHandle && titleInputHandle.asElement()) {
      ...
      await page.keyboard.type("5 Autonomous AI Agents That Run Your Business", { delay: 10 });
    ...
    // Fill caption text
    const captionText = fs.readFileSync(path.join(__dirname, 'urgent_claude_agents_post.txt'), 'utf8');
    ...
    await delay(8000);
    console.log("🎉 5 AUTONOMOUS AI AGENTS CAROUSEL POST PUBLISHED SUCCESSFULLY TO LINKEDIN!");
