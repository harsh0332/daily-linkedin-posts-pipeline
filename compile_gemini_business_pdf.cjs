...
(async () => {
    console.log("Launching Puppeteer to build Gemini Business Carousel PDF...");
    ...
    const outDir = path.resolve(__dirname, './user_uploads/gemini_business_output');
    fs.mkdirSync(outDir, { recursive: true });

    for (let i = 1; i <= 7; i++) {
        const slidePath = `file://${path.resolve(__dirname, `./carousel-routine/temp/gemini-business-branded/slide-0${i}.html`)}`;
    ...
    await browser.close();
    console.log("Gemini Business Carousel PDF compilation complete!");
})();
