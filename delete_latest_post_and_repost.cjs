...
(async () => {
  ...
  console.log("Launching browser to delete previous post & publish fixed carousel...");
  ...
  try {
    ...
    // STEP 1: Delete previous post from recent activity feed
    console.log("Navigating to user profile activity feed to delete previous post...");
    await page.goto('https://www.linkedin.com/in/harshchouksey/recent-activity/all/', { waitUntil: 'domcontentloaded', timeout: 30000 });
    await delay(5000);

    // Click 3 dots menu on the top post
    console.log("Clicking 3 dots control menu on top post...");
    const clickedMenu = await clickNativelyShadow(page, (root) => {
      const btns = Array.from(root.querySelectorAll('button'));
      return btns.find(b => (b.ariaLabel && b.ariaLabel.includes('Open control menu')) ||
                            (b.innerText && b.innerText.includes('Control menu')));
    });

    if (clickedMenu) {
      await delay(2000);
      console.log("Clicking 'Delete post' option...");
      const clickedDeleteOption = await clickNativelyShadow(page, (root) => {
        const items = Array.from(root.querySelectorAll('*'));
        return items.find(el => el.innerText && el.innerText.trim() === 'Delete post');
      });

      if (clickedDeleteOption) {
        await delay(2000);
        console.log("Confirming deletion...");
        const confirmedDelete = await clickNativelyShadow(page, (root) => {
          const btns = Array.from(root.querySelectorAll('button'));
          return btns.find(b => b.innerText && b.innerText.trim() === 'Delete');
        });
        if (confirmedDelete) {
          console.log("Successfully deleted previous post!");
          await delay(4000);
        }
      }
    } else {
      console.log("Control menu not clicked or post already deleted.");
    }

    // STEP 2: Publish the updated, flawless carousel PDF
    console.log("Navigating to LinkedIn feed to publish updated carousel...");
    ...
    console.log("Uploading fixed Carousel PDF:", pdfPath);
    ...
    await delay(8000);
    console.log("🎉 UPDATED FLAWLESS GEMINI BUSINESS CAROUSEL POST PUBLISHED SUCCESSFULLY!");
