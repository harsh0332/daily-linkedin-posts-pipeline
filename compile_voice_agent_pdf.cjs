...
(async () => {
    console.log("Launching Puppeteer to build Voice Agent Carousel PDF...");
    ...
    const outDir = path.resolve(__dirname, './user_uploads/voice_agent_output');
    fs.mkdirSync(outDir, { recursive: true });

    for (let i = 1; i <= 7; i++) {
        const slidePath = `file://${path.resolve(__dirname, `./carousel-routine/temp/voice-agent-branded/slide-0${i}.html`)}`;
    ...
    await browser.close();
    console.log("Voice Agent Carousel PDF compilation complete!");
})();
